...
def scrape_nk(url=None):
    ...
    # Phân loại để tránh quét trùng lặp giữa máy lọc không khí và máy hút ẩm
    if url:
        if "may-loc-khong-khi" in url:
            urls_to_scrape = [u for u in urls_to_scrape if "may-loc-khong-khi" in u or "aero-speaker" in u or "aero-furniture" in u or "aero-tower" in u]
        elif "may-hut-am" in url:
            urls_to_scrape = [u for u in urls_to_scrape if "may-hut-am" in u]
